import { Button } from "@/components/ui/button"
import { WhatsAppLink } from "@/components/whatsapp-link"
import { ArrowRight } from "lucide-react"

export function Hero() {
  return (
    <section className="relative h-screen flex items-center justify-center overflow-hidden">
      {/* Background Image */}
      <div className="absolute inset-0 z-0">
        <img
          src="/images/heropict.jpg"
          alt="Pria berkostum bespoke di Roma"
          className="w-full h-full object-cover object-[center_30%] opacity-30"
        />
        <div className="absolute inset-0 bg-gradient-to-b from-background/80 via-background/60 to-background" />
      </div>

      {/* Content */}
      <div className="relative z-10 container mx-auto px-6 text-center">
        <p className="text-sm tracking-[0.3em] uppercase text-muted-foreground mb-6">Sejak 1935</p>
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-light tracking-tight text-foreground mb-8 text-balance leading-[1.1]">
          Jas yang dijahit untuk sebuah warisan
          <br />

        </h1>
        <p className="text-lg md:text-xl font-light text-muted-foreground max-w-2xl mx-auto mb-12 leading-relaxed">
          Bespoke penuh <i>full</i> canvas dengan wool italia pilihan (Zegna, Dormeuil, Holland & Sherry). Dikerjakan tangan oleh keluarga penjahit generasi ketiga di jantung Jakarta.
          <br />

        </p>
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
          <a href="#heritage">
            <Button size="lg" className="group">
              Temukan Warisan Kami
              <ArrowRight className="ml-2 group-hover:translate-x-1 transition-transform" size={18} />
            </Button>
          </a>
          <WhatsAppLink source="hero">
            <Button size="lg" variant="outline">
              Jadwalkan Konsultasi
            </Button>
          </WhatsAppLink>
        </div>
      </div>

      {/* Scroll Indicator */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
        <div className="w-[1px] h-16 bg-foreground/20 animate-pulse" />
      </div>
    </section>
  )
}
